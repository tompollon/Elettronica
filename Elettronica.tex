...
\usepackage{pgfplots} %utility for plotting
...
\index{TO DO} look up the axis measures and ticks to make it realistic. 
\begin{center}
\begin{tikzpicture}
...
\end{equation}  
Where we introduce the \index{Electron mobility} Electron mobility $\mu$, that depends on $\bar{t}$, which is a material constant.
\begin{equation}
...
\begin{equation}
	\mu_p = 3\mu_n
\end{equation}
where $\mu_p$ is the \I{electron mobility} and $\mu_n$ is the \I{holes mobility} \index{Holes Mobility}. This doesn't surprise ourself because holes "move" in \I{valence band}, while electrons move in \I{conductance band}, in which the movement is "less obstructed by nuclei.  
